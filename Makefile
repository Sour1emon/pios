...
check-toolchain:
	@echo "Checking $(ARMGNU) supports AArch64..."
	@mkdir -p $(BUILD_DIR)
	@printf ".arch armv8-a\nmrs x0, mpidr_el1\n" > $(BUILD_DIR)/check.S
	-@$(ARMGNU)-gcc -c $(BUILD_DIR)/check.S -o $(BUILD_DIR)/check.o >/dev/null 2>&1 || (echo "Error: $(ARMGNU)-gcc does not support AArch64. Install an aarch64 toolchain and run 'make ARMGNU=aarch64-elf'"; exit 1)
